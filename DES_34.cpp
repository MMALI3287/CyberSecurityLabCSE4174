...
b64 finalPermutation(b64 input)
{
    b64 output;
...
vector<b48> generateSubkeys(b64 key)
{
    b56 permuted_key = permChoice1(key);
...
{
    b48 expanded_half = expansion(half_block);
...
vector<b64> msgToBlockOfBits(string msg)
{
    vector<b64> output;
    int i=0;
    b64 block;
...
vector<b64> desEncryptCBC(vector<b64> plaintext, vector<b48> subkeys, b64 iv) {
    vector<b64> ciphertext;
    b64 prevBlock = iv;
    for (b64 block : plaintext) {
...
vector<b64> desDecryptCBC(vector<b64> ciphertext, vector<b48> subkeys, b64 iv) {
...
int main()
{
    string msg,keyStr,ivStr;
    cout<<"Enter message to encrypt: ";
    getline(cin, msg);
    if(msg.length()%8 != 0)
    {
        cout<<"ERROR: Message length must be multiple of 8"<<endl;
        cout<<msg.length()%8;
        exit(2);
    }
    cout<<"Enter key: ";
    ...
    cout<<"Enter iv: ";
    cin>>ivStr;
    if(keyStr.length() != 16 && ivStr.length() != 16)
    {
        cout<<"Error: key and iv must be of length 16 and be hex";
        exit(2);
    }
    b64 key(stoull(keyStr, nullptr, 16));
    b64 iv(stoull(ivStr, nullptr, 16));

    vector<b64> msgBlocks = msgToBlockOfBits(msg);
    vector<b48> subkeys = generateSubkeys(key);
    vector<b64> ciphertext = desEncryptCBC(msgBlocks,subkeys,iv);
    cout<<"Cipher: ";
    for(b64 cipher: ciphertext)
    {
        cout<<bitToString(cipher);
    }
    cout<<endl;
    vector<b64> plaintext = desDecryptCBC(ciphertext,subkeys,iv);
    cout<<"Plain: ";
    for(b64 plain: plaintext)
    {
