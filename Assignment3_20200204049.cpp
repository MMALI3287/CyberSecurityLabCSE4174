...
string bitToCharacters(b64 input)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        char c = static_cast<char>(temp.to_ulong());
        output.push_back(c);
        input <<= 8;
    }
    return output;
...
int main()
{
    ...
    {
        cout << endl
             << "Encryption key and input vector must be of length 8 in ASCII" << endl;
    ...
    for (b64 cipherBlock : ciphertext)
    {
        cout << bitToCharacters(cipherBlock);
    }
    cout << endl;

    vector<b64> plaintext = cbcDesDecrypt(ciphertext, subkeys, initial64BitVector);
    cout << "Plain Text After Decryption: ";
    for (b64 plainBlock : plaintext)
    {
        cout << bitToCharacters(plainBlock);
    }
    cout << endl;
